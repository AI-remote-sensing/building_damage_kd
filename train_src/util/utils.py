...
def channel_five2two(mutli_tensor):
    # TODO(sujinhua): convert shape to support it can make
    return torch.cat(
        (
            torch.sum(mutli_tensor[:, :1, ...], dim=1).unsqueeze(1),
            torch.sum(mutli_tensor[:, 1:, ...], dim=1).unsqueeze(1),
        ),
        dim=1,
    )
...
def shift_image(img, shift_pnt):
    ...
    res = cv2.warpAffine(
        img, M, (img.shape[1], img.shape[0]), borderMode=cv2.BORDER_REFLECT_101
    )
    return res
